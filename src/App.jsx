...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => { unsubKegs(); unsubLocs(); unsubEvents(); unsubConfig(); };
  }, [user]);
  ...
  const returnKeg = (kegId) =>
    updateDoc(doc(db, 'users', user.uid, 'kegs', kegId), {
  ...
  const addLocation = async () => {
    ...
    await addDoc(collection(db, 'users', user.uid, 'locations'), {
      name: newLocationName, address: '', contact: ''
    });
    setNewLocationName('');
  ...
  const addEvent = async () => {
    ...
    await addDoc(collection(db, 'users', user.uid, 'events'), {
      name: newEventName, address: '', contact: ''
    });
    setNewEventName('');
    setCreationChoice(null);
  };
        ...
        <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto relative">
                  ...
                  {prices.length === 0 ? (
                    ...
                    <>
                      ...
                      <div className="border-t border-gray-100 pt-4 space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                          <button
                            onClick={() => handleCopyPriceList('bar')}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <button
                            onClick={() => handleGeneratePriceListPDF('bar')}
                            disabled={isGeneratingPriceListPDF}
                          ...
                          <button
                            onClick={() => handleGeneratePriceListPDF('event')}
                            disabled={isGeneratingPriceListPDF}
                            className={`py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center space-x-2 transition-all active:scale-95 border-2 ${isGeneratingPriceListPDF ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-white text-red-600 border-red-200 hover:bg-red-50'}`}
                          >
              ...
              <div className="space-y-3 pb-20">
                {kegs.map(k => (
                  <div key={k.id} className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between group">
                    ...
                    <div className="flex items-center space-x-2">
                      {k.status === 'entregado' && (
                        <button onClick={() => returnKeg(k.id)} className="p-2 text-gray-300 hover:text-green-600 transition-colors">
                          <Warehouse size={16} />
                        </button>
                      )}
                      <button onClick={() => deleteKeg(k.id)} className="p-2 text-gray-200 hover:text-red-600 transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
          ...
          {activeTab === 'settings' && (
            <div className="pt-4 space-y-6 animate-in fade-in slide-in-from-left-4 duration-300 pb-20">
              ...
              {/* SESIÓN */}
              <div className="bg-white p-5 rounded-[32px] shadow-sm border border-gray-100 flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Sesión activa</p>
                  <p className="font-bold text-sm text-gray-700 mt-0.5">{user.email}</p>
                </div>
                <button
                  onClick={handleLogout}
                  className="flex items-center space-x-2 bg-gray-100 text-gray-500 px-4 py-2 rounded-xl font-black uppercase text-[10px] hover:bg-red-50 hover:text-red-600 transition-colors active:scale-95"
                >
                  <LogOut size={14} />
                  <span>Salir</span>
                </button>
              ...
              <div className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowAddPrice(!showAddPrice)} className="bg-black text-white p-2 rounded-xl active:scale-90 shadow-lg">
                    <Plus size={16} />
                  </button>
                </div>
                {showAddPrice && (
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 space-y-3 animate-in slide-in-from-top-2">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        ...
                        <input type="number" placeholder="Ej: 50" className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-red-600" value={newPrice.capacity} onChange={e => setNewPrice({ ...newPrice, capacity: e.target.value })} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[8px] font-black uppercase text-gray-400 ml-1">Estilo</label>
                        <select className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-red-600" value={newPrice.style} onChange={e => setNewPrice({ ...newPrice, style: e.target.value })}>
                          <option value="">Estilo...</option>
                          {styles.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        ...
                        <input type="number" placeholder="Ej: 12000" className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-red-600" value={newPrice.priceBar} onChange={e => setNewPrice({ ...newPrice, priceBar: e.target.value })} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[8px] font-black uppercase text-gray-400 ml-1">Precio Evento $</label>
                        <input type="number" placeholder="Ej: 15000" className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-red-600" value={newPrice.priceEvent} onChange={e => setNewPrice({ ...newPrice, priceEvent: e.target.value })} />
                      </div>
                    </div>
                    <div className="flex space-x-2">
